...
class Shell:
    ...
    def _get_completions(self, text: str) -> list[str]:
        ...
        prefix = (text or (line[beg:end] if beg < len(line) else "")).lower()
        completions: list[str] = []
        before_cursor = line[:beg]
        is_first_token = not before_cursor.strip() or before_cursor.strip().split() == [before_cursor.strip()]
        if is_first_token or (text and text.startswith("$")):
                ...
                for d in path_env.split(os.pathsep):
                    ...
                    try:
                        for name in os.listdir(d):
                            if name.lower().startswith(prefix) and os.access(os.path.join(d, name), os.X_OK):
            ...
            try:
                ...
                for name in os.listdir(search_dir):
                    if name.startswith(file_part or prefix):
